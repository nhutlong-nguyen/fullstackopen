...
const mostLikes = (blogs) => {
    ...
    const likesByAuthor = _(blogs)
      ...
      .tap((grouped) => console.log('After groupBy:', grouped)) // Log the grouped blogs.
      .map((posts, author) => ({
        author: author,
        likes: _.sumBy(posts, 'likes') // Sum the 'likes' for each author's posts.
      }))
      .tap((summed) => console.log('After map/sumBy:', summed)) // Log the summed likes.
      .value() // Extract the value from the chain.
  
    const topAuthor = _.maxBy(likesByAuthor, 'likes')
    console.log('Most liked author:', topAuthor) // Log the author with most likes.
